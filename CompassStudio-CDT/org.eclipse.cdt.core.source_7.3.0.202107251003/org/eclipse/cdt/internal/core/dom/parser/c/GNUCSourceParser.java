 ...
 *     Alexander Ny��en (itemis AG) - bug 475908
 *******************************************************************************/
...
public class GNUCSourceParser extends AbstractGNUSourceCodeParser {
	...
	private IASTInitializerClause initClause() throws EndOfFileException, BacktrackException {
		...
		for (;;) {
			...
			if (designator == null) {
				...
				// depending on value of skipTrivialItemsInCompoundInitializers initializer may
				// be null
	...
	private List<? extends ICASTDesignator> designatorList() throws EndOfFileException, BacktrackException {
		...
		// fix for 84176: if reach identifier and it's not a designator then return
		// empty designator list
	...
	private IASTExpression expression(final ExprKind kind) throws EndOfFileException, BacktrackException {
		...
		loop: while (true) {
			...
			case IToken.tQUESTION:
				...
				// Precedence: 25 is lower than precedence of logical or; 0 is lower than
			...
			case IToken.tCOLON:
				...
				// <logical-or> ? <expression> : <assignment-expression>
				// Precedence: 0 is lower than precedence of expression; 15 is lower than
...
//					identifier = identifier();
				...
				case IToken.t_half:
					if (encounteredTypename)
						break declSpecifiers;
					simpleType = IASTSimpleDeclSpecifier.t_half;
					encounteredRawType = true;
					endOffset = consume().getEndOffset();
					break;
				case IToken.t_event_t:// CUSTOMIZATION FOR OPENCL
					if (encounteredTypename)
						break declSpecifiers;
					simpleType = IASTSimpleDeclSpecifier.t_event_t;
					encounteredRawType = true;
					endOffset = consume().getEndOffset();
					break;
				...
				case IToken.tEOC:
					...
					// CUSTOMIZATION
					char[] simpleID = identifier.getSimpleID();
					String s = String.valueOf(simpleID);
					encounteredTypename = true;
					// CUSTOMIZATION
					if (s.equals("__lsram") || s.equals("lsram")) {
						storageClass = IToken.t_volatile;
						endOffset = consume().getEndOffset();
					}
					if (s.equals("__global") || s.equals("global")) {
						storageClass = IToken.t_volatile;
						endOffset = consume().getEndOffset();
					}
					if (s.equals("__private") || s.equals("private")) {
						storageClass = IToken.t_volatile;
						endOffset = consume().getEndOffset();
					}
					if (s.equals("__constant") || s.equals("constant")) {
						storageClass = IToken.t_volatile;
						endOffset = consume().getEndOffset();
					}
					if (s.equals("local") || s.equals("local")) {
						storageClass = IToken.t_volatile;
						endOffset = consume().getEndOffset();
					}
					if (s.equals("__lsram0") || s.equals("lsram0")) {
						storageClass = IToken.t_volatile;
						endOffset = consume().getEndOffset();
					}
					if (s.equals("__lsram1") || s.equals("lsram1")) {
						storageClass = IToken.t_volatile;
						endOffset = consume().getEndOffset();
					}
					if (s.equals("__gsram0") || s.equals("gsram0")) {
						storageClass = IToken.t_volatile;
						endOffset = consume().getEndOffset();
					}
					if (s.equals("__gsram1") || s.equals("gsram1")) {
						storageClass = IToken.t_volatile;
						endOffset = consume().getEndOffset();
					}
...
//					}
	...
	protected ICASTCompositeTypeSpecifier structOrUnionSpecifier() throws BacktrackException, EndOfFileException {
		...
		// if __attribute__ or __declspec occurs after struct/union/class and before the
		// identifier
		...
		// if __attribute__ or __declspec occurs after struct/union/class identifier and
		// before the { or ;
