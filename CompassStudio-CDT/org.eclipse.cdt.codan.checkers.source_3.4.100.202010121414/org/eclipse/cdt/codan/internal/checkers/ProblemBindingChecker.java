					...
					try {
						...
						if (binding instanceof IProblemBinding) {
							...
							if (id == IProblemBinding.SEMANTIC_INVALID_TEMPLATE_ARGUMENTS) {
								...
								// argument list to be underlined. That way we can see which argument is
								// invalid.
...
//							}
							...
							if (isFunctionCall(name, parentNode)) {
								...
								if (problemBinding.getID() == IProblemBinding.SEMANTIC_NAME_NOT_FOUND) {
									// OpenCL vector component's valid name patterns
									String patterns[] = { "S[0-9a-fA-F]+", "s[0-9a-fA-F]+", "[xyzw]+", "lo", "even",
											"hi", "odd" };
									for (String pattern : patterns) {
										Pattern r = Pattern.compile(pattern);
										Matcher m = r.matcher(name.toString());
										if (m.find())
											return PROCESS_CONTINUE;
									}
								}
...
//								if (name.toString().equals("half") || name.toString().equals("event_t")) {
//									return PROCESS_CONTINUE;
//								} // CUSTOMIZATION FOR OPENCL
								reportProblem(ERR_ID_TypeResolutionProblem, name, name.getRawSignature(),
										contextFlagsString);
	...
	private void handleMemberProblem(IASTName name, IASTNode parentNode, IProblemBinding problemBinding,
		...
		// An IASTFieldReference corresponds to a method if it's the first child in the
		// parent function call expression
		// For example,
		// func(x.y()); the field reference is first in the x.y() function call
		// expression -> y is a method
		// func(x.y); the field reference is second in the func(x.y) function call
		// expression, func is first as a Id Expression -> y is a field
