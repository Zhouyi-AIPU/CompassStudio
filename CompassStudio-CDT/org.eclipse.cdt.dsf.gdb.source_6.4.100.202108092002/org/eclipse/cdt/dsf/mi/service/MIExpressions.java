...
public class MIExpressions extends AbstractDsfService implements IMIExpressions, ICachingService {
	...
	protected static class IndexedPartitionDMC extends MIExpressionDMC implements IIndexedPartitionDMContext {
		...
		private static ExpressionInfo createExpressionInfo(ExpressionInfo parentInfo, int index, int length) {
			...
			// CUSTOMIZATION :type get it from user result

			String expression1 = String.format("*(%s(*)[%d])&%s[%d]", getType(), Integer.valueOf(length), parentInfo.getFullExpr(),
					Integer.valueOf(index));
			return new ExpressionInfo(expression1, expression1);
		}
											...
											protected void handleSuccess() {
												if (IExpressionDMData.BasicType.array
													...
													rm.setData(getTopLevelIndexedPartitions((MIExpressionDMC) exprCtx,
															realNumChildren, startIndex, length, getData().getTypeName()));
													rm.done();
												} else {
													getRealSubExpressions(exprCtx, startIndex, length, rm);
												}
	...
	private IndexedPartitionDMC[] getTopLevelIndexedPartitions(MIExpressionDMC exprCtx, int realNumChildren,
		...
		for (int i = 0; i < children.length; ++i) {
			...
			children[i] = createIndexedPartition(exprCtx, index, partLength, type);
			index += partLength;
		}
	...
	// CUSTOMIZATION type:use define, get it from the result
	private IndexedPartitionDMC createIndexedPartition(MIExpressionDMC parentExpr, int index, int length, String type) {

		// if type is "", called from getIndexedPartitionChildren.this methos don't need to set type
		if (!type.equals("")) {
			type = type.substring(0, type.indexOf(" ")).trim();
			IndexedPartitionDMC.setType(type);
		}
