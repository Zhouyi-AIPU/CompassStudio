...
public class LlvmEnvironmentVariableSupplier implements IConfigurationEnvironmentVariableSupplier {
	...
	public static final String ENV_VAR_NAME_LIBRARY_PATH = "NPU_LIB_SEARCH_PATH"; //$NON-NLS-1$
	...
	public static String getBinPath() {
		...
		// CUSTOMIZATION
		String llvmBinPath = findBinDir(ENV_VAR_NAME_LLVM_BIN, "bin");

		if (llvmBinPath != null) {
			String llvmLibPath = llvmBinPath.substring(0, llvmBinPath.lastIndexOf(File.separator)) + File.separator
					+ "lib";
			setLlvmEnvironmentVariable(ENV_VAR_NAME_LIBRARY_PATH, llvmLibPath);
		}
		setLlvmEnvironmentVariable(LD_LIBRARY_PATH, GlobalPreference.getPathSimulatorLib());

		// setLlvmEnvironmentVariable(ENV_VAR_NAME_LLVM_BIN, llvmBinPath);

		return llvmBinPath; // $NON-NLS-1$
	}
	...
	private static String getBinDirIfLlvm_ar(String binPathTemp) {
		...
		if (new Path(binPathTemp).toFile().isDirectory()) {
			...
			// CUSTOMIZATION
			// clang-8 -> aipucc
			String llvm_executable = "aipucc"; //$NON-NLS-1$
